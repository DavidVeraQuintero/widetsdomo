...
function HumoGasModal({ alert, name, config, onConfigChange, onClose }) {
  const col = alert ? '#ef4444' : '#22c55e';
  const icons = useWidgetIcons('sensor-humo', config.icons);
  return (
    <ModalBase
      title="🔥 Sensor Humo / Gas"
      onClose={onClose}
      borderColor={col}
    >
      ...
      <div style={{ textAlign:'center', fontSize:14, color:'var(--text-primary)', fontWeight:alert ? 700 : 400, marginBottom:16 }}>{alert ? '⚠ ALERTA DETECTADA' : '✓ Todo normal'}</div>
      <div style={{ display:'flex', justifyContent:'center', marginBottom:20 }}>
        <div style={{ width:20, height:20, borderRadius:'50%', background:col, boxShadow:alert ? `0 0 16px ${col}` : 'none' }} />
      </div>
      <button className="w-btn" style={{ width:'100%', ...(alert ? { borderColor:'#ef4444', color:'var(--text-primary)' } : {}) }}
        onClick={() => onConfigChange({ alert: !alert })}
        onMouseDown={e => e.stopPropagation()}>
        Simular {alert ? 'calma' : 'alerta'}
      </button>
      <IconSection typeId="sensor-humo" config={config} onConfigChange={onConfigChange} resolvedIcons={icons} />
    </ModalBase>
  );
}
...
export default function SensorHumoGas({ size, config, onConfigChange, accentColor }) {
  ...
  const Modal = modal && (
    <HumoGasModal alert={alert} name={name} config={config} onConfigChange={patchConfig} onClose={() => setModal(false)} />
  );
  ...
  if (size === '1x1') return (
    <div className="w-body" style={{ alignItems:'center', justifyContent:'center' }}>
      <div style={{ display:'flex', flexDirection:'column', alignItems:'center', gap:2 }}>
        ...
        <span style={{ fontSize:12, color:'var(--text-primary)' }}>{alert ? '⚠ ALERTA' : '✓ OK'}</span>
      </div>
      {Modal}
    </div>
  );
  ...
  if (size === '1x2') return (
    <div className="w-body w-center">
      ...
      <div className="w-status" style={{ color:'var(--text-primary)', fontWeight:alert ? 700 : 400 }}>{alert ? '⚠ ALERTA DETECTADA' : '✓ Todo normal'}</div>
      <button className="w-btn w-btn-sm" onClick={e => { e.stopPropagation(); toggle(); }} onMouseDown={e => e.stopPropagation()}>Simular {alert ? 'calma' : 'alerta'}</button>
      {Modal}
    </div>
  );
  ...
  return (
    <div className="w-row-body">
      ...
      <div className="w-info">
        <div className="w-name">{name}</div>
        <div className="w-status" style={{ color:'var(--text-primary)' }}>{alert ? '⚠ Alerta detectada' : '✓ Todo normal'}</div>
      </div>
      <div style={{ width:14, height:14, borderRadius:'50%', background:col, boxShadow:alert ? `0 0 12px ${col}` : 'none', flexShrink:0 }} />
      {Modal}
    </div>
  );
}
