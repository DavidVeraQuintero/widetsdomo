...
function TermostatoModal({ target, current, name, config, onConfigChange, onClose, accentColor }) {
  ...
  return (
    ...
    >
      ...
      <div style={{ textAlign:'center', fontSize:11, color:'var(--text-secondary)', marginBottom:16 }}>temperatura actual</div>
      <div style={{ fontSize:9, color:'var(--text-secondary)', textTransform:'uppercase', letterSpacing:1, marginBottom:6 }}>Objetivo · {target}°C</div>
      <Slider value={target} min={15} max={30} onChange={v => onConfigChange({ target: v })} showVal={false} />
      <div style={{ textAlign:'center', fontSize:11, color:'var(--text-secondary)', marginTop:8, fontWeight:600 }}>{heating ? '🔥 Calentando' : '❄ Refrigerando'}</div>
...
export default function Termostato({ size, config, onConfigChange, accentColor }) {
  ...
  if (size === '2x1') return (
    <div className="w-row-body">
      <span style={{ cursor:'pointer', display:'flex', alignItems:'center' }} {...longPress}>
        <SvgIcon id={heating ? icons.heating : icons.idle} size={26} color="var(--icon-on)" className="icon-glow" />
      </span>
      ...
      <div style={{ textAlign:'right' }}>
        ...
        <div style={{ fontSize:9, color:'var(--text-secondary)' }}>→{target}°</div>
      </div>
      {Modal}
    </div>
  );
  ...
  return (
    <div className="w-body">
      <div className="w-row">
        <div className="w-label">🌡 Termostato</div>
        <span style={{ fontSize:10, color:'var(--text-secondary)', fontWeight:600 }}>{heating ? '🔥 Calentando' : '❄ Refrigerando'}</span>
      </div>
      ...
      <div style={{ display:'flex', alignItems:'center', gap:6 }}>
        <span style={{ fontSize:10, color:'var(--text-secondary)', flexShrink:0 }}>→{target}°</span>
        <Slider value={target} min={15} max={30} onChange={setTarget} showVal={false} />
      </div>
      {Modal}
    </div>
