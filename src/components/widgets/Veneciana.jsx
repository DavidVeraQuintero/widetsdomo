  ...
  return (
    <ModalBase title="Veneciana" onClose={onClose} borderColor={accentColor}>
      ...
      <div style={{ marginTop: 12 }}>
        <div style={{ fontSize: 9, color: 'var(--text-secondary)', textTransform: 'uppercase', letterSpacing: 1, marginBottom: 6 }}>Ángulo lamas · {angle}°</div>
...
export default function Veneciana({ size, config, onConfigChange, accentColor }) {
  ...
  if (size === '1x2') return (
    <div style={{ height: '100%', display: 'flex', flexDirection: 'column', padding: '10px', gap: 6, overflow: 'hidden' }}>
      ...
      <div style={{ fontSize: 9, color: 'var(--text-secondary)', marginBottom: 2 }}>Posición</div>
  ...
  return (
    <div style={{ height: '100%', display: 'flex', flexDirection: 'column', padding: '10px', gap: 6, overflow: 'hidden' }}>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
        <div className="w-label">Veneciana</div>
        <span style={{ fontSize: 11, color: 'var(--text-secondary)', fontWeight: 700 }}>{position}% · {angle}°</span>
      </div>
      <div className="w-name">{name}</div>
      <div style={{ flex: 1, cursor: 'pointer', minHeight: 0 }} {...longPress}>
        <VenetianVisual position={position} angle={angle} color={accentColor} height="100%" />
      </div>
      <div style={{ fontSize: 9, color: 'var(--text-secondary)' }}>Posición</div>
      <Slider value={position} onChange={setPos} showVal={false} />
      <div style={{ fontSize: 9, color: 'var(--text-secondary)' }}>Ángulo lamas</div>
