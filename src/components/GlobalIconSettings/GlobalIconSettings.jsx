      ...
      {categories.map(cat => {
        ...
        return (
          <div key={cat} style={{ marginBottom: 18 }}>
            <div style={{ fontSize: 12, color: 'rgba(255,255,255,0.75)', textTransform: 'uppercase', letterSpacing: 0.5, marginBottom: 8 }}>{cat}</div>
            {items.map(def => {
              const meta = WIDGET_ICON_META[def.id];
              if (!meta) return null;
              const hasOverride = !!state.globalIcons?.[def.id];
              return (
                <div key={def.id} style={{ marginBottom: 6, background: 'rgba(255,255,255,0.06)', borderRadius: 8, padding: '7px 9px', border: '1px solid rgba(255,255,255,0.12)' }}>
                  <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: meta.states.length > 1 ? 5 : 0 }}>
                    <span style={{ fontSize: 12, color: 'white' }}>{def.name}</span>
                    {hasOverride && (
                      <button className="w-btn" style={{ fontSize: 12, padding: '2px 8px', borderColor: '#ef4444', color: '#ef4444' }}
                  ...
                  {meta.states.map(stateKey => (
                    <div key={stateKey} style={{ padding: '3px 0' }}>
                      {meta.states.length > 1 && (
                        <div style={{ fontSize: 12, color: 'rgba(255,255,255,0.65)', marginBottom: 3 }}>
                      ...
                      <div style={{ display: 'flex', alignItems: 'center', gap: 5 }}>
                        <div style={{ width: 22, height: 22, borderRadius: 5, background: 'rgba(255,255,255,0.10)', display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0 }}>
                          <SvgIcon id={getIcon(def.id, stateKey)} size={13} color="rgba(255,255,255,0.85)" />
                        </div>
                        <span style={{ fontSize: 12, color: 'rgba(255,255,255,0.60)', flex: 1, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>
                          {getIcon(def.id, stateKey)}
                        </span>
                        <button className="w-btn" style={{ fontSize: 12, padding: '2px 8px', flexShrink: 0 }}
